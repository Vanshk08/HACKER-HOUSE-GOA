...
def extract_policy_context(state: dict[str, Any]) -> dict[str, Any]:
    ...
    # If exposure was 0.0 but flagged_txn_id exists in evidence, inspect evidence
    if exposure == 0.0 and state.get("flagged_txn_id"):
        for ev in evidence:
...
def rule_r1(context: dict[str, Any]) -> RuleResult:
    ...
    # Applies when fraud probability < 0.70, not confirmed fraud, and customer hasn't confirmed/denied yet
    if prob < 0.70 and verdict != "confirmed_fraud" and status not in ("confirmed", "denied"):
    ...
    return RuleResult(
        ...
        matched=False,
        actions=[],
        rationale="Condition not met: fraud probability >= 0.70, confirmed fraud, or customer response already resolved.",
    )
...
def evaluate_policy(state_or_context: dict[str, Any]) -> PolicyDecision:
    ...
    decision: PolicyDecision = {
        ...
        "exposure": context.get("exposure", 0.0),
        "affected_txn_ids": context.get("affected_txn_ids", []),
