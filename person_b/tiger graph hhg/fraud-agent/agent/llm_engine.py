...
class AutonomousAssessmentLLM:
    ...
    def invoke(self, messages: list[Any]) -> AssessmentSchema:
        ...
        # get_customer_regions output in content: 'region_codes': ['...', '...']
        is_region_established = False
        if billing_region:
            # Look for region code in region_codes list
        ...
        if has_explicit_denial:
            # Cardholder verified denial via outreach
            ...
            # Cardholder confirmed authorization
            verdict = "legitimate"
            fraud_prob = 0.05
            fraud_type = None
            ...
            # Analyst request: unusual device profile pattern
            ...
            # Inbound customer report, but outreach response status is unknown
            verdict = "uncertain"
            fraud_prob = 0.52
            fraud_type = None
            supporting = [
            ...
            if is_region_established:
                # Billing region is established in customer profile (e.g. HHG-001)
                verdict = "uncertain"
                fraud_prob = 0.42
                fraud_type = None
                supporting = [
                ...
                # Unseen/novel region or high risk score
