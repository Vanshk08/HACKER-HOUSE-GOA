...
def run_all_cases(
    ...
    if not selected_provider:
        raise ValueError(
            "No LLM provider configured. You must set LLM_PROVIDER=openai|anthropic|mock "
...
if __name__ == "__main__":
    ...
    parser.add_argument("--provider", default=os.getenv("LLM_PROVIDER"), help="LLM Provider: openai, anthropic, or mock")
    parser.add_argument("--csv", default="data/case_pack.csv", help="Path to case_pack.csv")
    parser.add_argument("--out", default="results", help="Output directory")
    args = parser.parse_args()

    summary = run_all_cases(csv_path=args.csv, output_dir=args.out, provider=args.provider)
